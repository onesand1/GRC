...
#' @param expression a gene name or a vector of gene names for one gene or many genes.default expression=2,means caculate the differential level for firt gene.
#' @param exp_type the expression data formula mode. including 'RPKM','FPKM','TPM'.

SL<-function(data,group="group",expression=2,exp_type="rpkm",plot=TRUE){
  if(plot){
  ...
  result=list(gene=expression,P=c(),FC=c(),q_P=c(),FC_type="",method="")
   if(dim(data)[1]<4){
    ...
    if(length(gg)!=2){
      stop("The group needs to have 2 levels (2 groups) !")
    }
